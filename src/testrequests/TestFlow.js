...
import LabConsultationResultDetail from "./LabConsultationResultDetail";
import AnalysisConsultationResultDetail from "./AnalysisConsultationResultDetail";
...
function TestFlow(props) {
    ...
    const {orderId} =testRequest
    const [rows,setRows] = useState([])

    let labResult=null
    let labConsultation=null
    let analysisConsultation=null
    ...
    if(testRequest.status === "COMPLETED"  )
        labConsultation=testRequest.labConsultation;

    
    if(testRequest.status === "COMPLETED"  )
    analysisConsultation=testRequest.analysisConsultation;

    function reloadRows(){

        doGetTestRequestFlow(orderId)
            .subscribe((results) => {
                console.log(results)
                setRows(results)
        ...
        <React.Fragment>
            ...
            <Grid container spacing={2}  className={classes.formContainer} >
                ...
                <Grid item xs={6} >
                    ...
                    <LabConsultationResultDetail labConsultation={labConsultation}></LabConsultationResultDetail>
                </Grid>
                <Grid item xs={6} >

                    <AnalysisConsultationResultDetail analysiConsultation={analysisConsultation}></AnalysisConsultationResultDetail>
                </Grid>
            </Grid>
